...
def save_disclosure_detail(rcept_no: str):
    if os.path.isfile(os.path.join("disclosures", f"{rcept_no}.xml")):
    ...
    with zipfile.ZipFile(io.BytesIO(response.content), 'r') as zip_ref:
        ...
        zip_ref.extract(f"{rcept_no}.xml","disclosures")
...
def get_contents_of_business(rcept_no):
    ...
    with open(os.path.join("disclosures", f"{rcept_no}.xml"), 'r') as file:
        content = file.read()
...
def get_contents_of_investor_protection(rcept_no):
    save_disclosure_detail(rcept_no)
    with open(os.path.relpath(os.path.join("disclosures", f"{rcept_no}.xml")), 'r') as file:
        content = file.read()
...
def replace_reserved_word(rcept_no: str):
    ...
    with open(os.path.join("disclosures", f"{rcept_no}.xml"), 'w') as file:
